...
function generateEmailHtml(template: EmailTemplate, shortcodeData: Record<string, string>): string {
  ...
  const buttonText = template.tracking_button_text ? replaceShortcodes(template.tracking_button_text, shortcodeData) : 'View Your Delivery';
  ...
  const deliveryBoxHtml = `<div class="delivery-box"><h3>Your Digital Delivery is Ready!</h3><p>Click the button below to securely view your code/account details</p></div>`;
...
function generateDefaultEmailHtml(shortcodeData: Record<string, string>): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>${styles}</style></head><body><div class="container"><div class="header"><h1>Your Order Has Been Delivered!</h1></div><div class="content"><p>Dear <strong>${shortcodeData.customer_name}</strong>,</p><p>Great news! Your digital order has been delivered and is ready for you.</p><div class="delivery-box"><h3>Your Digital Delivery is Ready!</h3><p>Click the button below to securely view your code/account details</p></div><div class="order-info"><p><strong>Order ID:</strong> #${shortcodeData.order_id}</p><p><strong>Order Total:</strong> $${shortcodeData.order_total}</p><p><strong>Status:</strong> <span class="status-badge">Delivered</span></p></div><div style="text-align:center;margin:25px 0"><a href="${shortcodeData.tracking_url}" class="cta">View Your Delivery</a></div><p>Best regards,<br><strong>Golden Bumps Team</strong></p></div><div class="footer"><p>This is an automated message from Golden Bumps.</p><div style="border-top:1px solid #e5e7eb;margin-top:15px;padding-top:15px;font-size:11px;color:#9ca3af"><p><strong>Refund Policy:</strong> Refunds are processed within 24-48 hours after verification.</p><p><strong>Delivery:</strong> Digital products are delivered instantly via email and your account dashboard.</p><p><strong>Support Hours:</strong> 10AM - 2AM Everyday</p></div></div></div></body></html>`;
}
